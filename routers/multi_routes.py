...
async def premium(male_files: List[UploadFile] = File(...), female_files: List[UploadFile] = File(...), logger: logging.Logger = Depends(get_logger), background_tasks: BackgroundTasks = BackgroundTasks()):
    try:
        ...
        if not male_filenames or not female_filenames:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to upload files")
        ...
        # 템플릿 이미지 경로들을 가져옴
        all_template_img_paths = [os.path.join("template", filename) for filename in os.listdir(
            "template") if filename.endswith('.png')]
        ...
        task_queue.enqueue(faceswap_and_update_status_premium, unique_id,
                           selected_template_img_paths, male_filenames, female_filenames, logger)
        ...
        if all_result_files:
            ...
            for result_file in all_result_files:
                with open(result_file, 'rb') as f:
                    file_content = f.read()
                    files_list.append(('files', (os.path.basename(
                        result_file), file_content, 'application/octet-stream')))

            payload = {'task_id': unique_id, 'status': 'completed'}
            requests.post(SPRING_URL+'/api/v1/saveAiImages',
                          data=payload, files=files_list)
            ...
            requests.post(
                SPRING_URL+'/api/v1/saveAiImages', json=payload)
