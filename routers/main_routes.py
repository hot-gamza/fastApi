...
async def index(male_files: List[UploadFile] = File(...), female_files: List[UploadFile] = File(...), logger: logging.Logger = Depends(get_logger),  background_tasks: BackgroundTasks = BackgroundTasks()):
            ...
            with open(result_filepath, 'rb') as f:
                ...
                if response.status_code == 200:
                    if response.text == 'success':
                        logger.info(f"Successfully sent the file for task {unique_id}")
                    else:
                        logger.error(f"Failed to send the file for task {unique_id}. Server responded with: {response.text}")
                else:
                    logger.error(f"Failed to send the file for task {unique_id}. HTTP status code: {response.status_code}")
